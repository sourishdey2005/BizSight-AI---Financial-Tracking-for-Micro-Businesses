...
const items: InventoryItem[] = [
  { id: '1', name: 'Hammer X10', stock: 5, costPerUnit: 12.00, salePrice: 25.00, category: 'Tools' },
  { id: '2', name: 'Screws 50pk', stock: 120, costPerUnit: 2.50, salePrice: 6.99, category: 'Hardware' },
  { id: '3', name: 'Power Drill V2', stock: 2, costPerUnit: 85.00, salePrice: 149.99, category: 'Tools' },
  { id: '4', name: 'Measuring Tape', stock: 15, costPerUnit: 4.00, salePrice: 12.00, category: 'Hardware' },
];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-slate-900">$18,450.00</p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">${item.costPerUnit.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">${item.salePrice.toFixed(2)}</td>
                  <td className="px-6 py-4">
